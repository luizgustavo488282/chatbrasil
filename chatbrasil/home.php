...
if (isset($_GET['action'])) {
    ...
    switch ($action) {
        case 'search_users':
            $q = "%".($_POST['q'] ?? '')."%";
            // Puxa também o campo is_verified da tabela users
            $stmt = $pdo->prepare("SELECT id, username, avatar, is_verified FROM users WHERE username LIKE ? AND id != ?");
        ...
        case 'get_friends':
            // 1. Puxa os amigos e contatos diretos incluindo o status is_verified do amigo
            $stmt = $pdo->prepare("
            ...
            $stmt_grupos = $pdo->prepare("
                SELECT g.id, g.nome AS username, NULL AS avatar, 0 AS unread_count, 1 AS is_group, 0 AS is_verified
                FROM grupos g 
...
document.addEventListener("DOMContentLoaded", function() {
    // CONEXÃO COM O CONTADOR AO VIVO (Registra o hit de acesso legítimo vindo da home)
...
function renderChats() {
    ...
    filteredList.forEach(item => {
        ...
        // Verifica se a pessoa tem o selo ativo no banco de dados
        let verifiedBadge = '';
        if (!isGroup && parseInt(item.is_verified) === 1) {
            verifiedBadge = `<i class="fa-solid fa-circle-check" style="color: #5288c1; margin-left: 4px; font-size: 14px;" title="Verificado"></i>`;
        }
        ...
        chatItem.innerHTML = `
            ...
            <div class="chat-info">
                <div class="chat-text-wrapper">
                    <div class="chat-name">${safeUsername}${verifiedBadge} ${isGroup ? '<span style="font-size:11px; color:var(--blue); font-weight:normal;">(Grupo)</span>' : ''}</div>
...
function searchUsers(query) {
    ...
    .then(users => {
        ...
        users.forEach(u => {
            ...
            // Verifica o selo de verificação também na tela de busca
            let verifiedBadge = '';
            if (parseInt(u.is_verified) === 1) {
                verifiedBadge = `<i class="fa-solid fa-circle-check" style="color: #5288c1; margin-left: 4px; font-size: 14px;" title="Verificado"></i>`;
            }
            ...
            item.innerHTML = `
                ...
                <div class="chat-info">
                    <div class="chat-text-wrapper">
                        <div class="chat-name">${escapeHtml(u.username)}${verifiedBadge}</div>
                    </div>
