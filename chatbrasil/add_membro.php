...
// Captura o ID do grupo vindo da URL
$grupo_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
if ($grupo_id <= 0) {
    die("Erro: ID do grupo inválido.");
}
...
// Processar a adição de membros no grupo usando PDO
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['adicionar_membros'])) {
    $amigos_selecionados = isset($_POST['amigos']) ? $_POST['amigos'] : [];
    $total_selecionado = count($amigos_selecionados);

    // Verifica quantos membros o grupo já possui atualmente
    try {
        $stmt_qtd = $pdo->prepare("SELECT COUNT(*) FROM grupo_membros WHERE grupo_id = ?");
        $stmt_qtd->execute([$grupo_id]);
        $membros_atuais = $stmt_qtd->fetchColumn();
    } catch (PDOException $e) {
        $membros_atuais = 0;
    }

    if (empty($amigos_selecionados)) {
        $erro = "Por favor, selecione ao menos um amigo para adicionar.";
    } elseif (($membros_atuais + $total_selecionado) > $limite_membros) {
        $erro = "Seu plano ($nome_cargo_formatado) permite no máximo $limite_membros pessoas por grupo. O grupo já tem $membros_atuais e você tentou colocar mais $total_selecionado.";
    } else {
        try {
            $pdo->beginTransaction();

            // Preparação do comando de inserção para evitar duplicidade na execução do loop
            $stmt_membro = $pdo->prepare("INSERT INTO grupo_membros (grupo_id, user_id) VALUES (?, ?)");

            // Adicionar os Amigos selecionados
            foreach ($amigos_selecionados as $amigo_id) {
                // Checa se o usuário já não faz parte do grupo antes de inserir
                $stmt_check = $pdo->prepare("SELECT COUNT(*) FROM grupo_membros WHERE grupo_id = ? AND user_id = ?");
                $stmt_check->execute([$grupo_id, $amigo_id]);
            ...
            $erro = "Erro ao adicionar membros. Certifique-se de que a tabela 'grupo_membros' está configurada corretamente.";
        }
    }
}

// Buscar lista de amigos vinculados utilizando o PDO - RESOLVIDO: GROUP BY u.id remove duplicidades da lista
$amigos_lista = [];
try {
    $query_friends = "
        SELECT u.id, u.username AS nome FROM friends f 
    ...
    try {
        $stmt_b = $pdo->prepare("SELECT id, username AS nome FROM users WHERE id != ? GROUP BY id LIMIT 100");
...
<head>
    ...
    <title>Adicionar Membros — Telegram Dark</title>
    <style>
        * { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, sans-serif; -webkit-font-smoothing: antialiased; }
        ...
        .friend-name { font-size: 15px; color: #ffffff; font-weight: 500; flex: 1; }
...
<div class="app-container">
    <div class="header">
        ...
        <div class="user-welcome">Olá, <b><?php echo htmlspecialchars($nome_usuario_logado); ?></b>! Selecione quem deseja trazer para o grupo.</div>
    </div>
    ...
    <div class="content">
        ...
        <form method="POST" action="" id="membrosForm" style="display: flex; flex-direction: column; flex: 1;">
    ...
    <div class="footer-btn">
        <button type="submit" form="membrosForm" name="adicionar_membros" class="btn-submit">Confirmar Integrantes</button>
    </div>
</div>
